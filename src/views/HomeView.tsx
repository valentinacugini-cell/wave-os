...
export default function HomeView({ seed, currentUser, onClienteClick }: HomeProps) {
  ...
  const filteredClienti = useMemo(() => {
    ...
    return [...list].sort((a, b) => {
      ...
      if (sortMode === 'scadenza') {
        const da = a.giorni ?? 9999
        const db = b.giorni ?? 9999
        return da - db
      }
  ...
  function getAlertBadge(c: typeof filteredClienti[0]) {
    ...
    if (c.giorni <= 30) return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold"
        style={{ background: '#FFEBEE', color: '#C62828' }}>
        🔴 {c.giorni}gg
      </span>
    )
    if (c.giorni <= 60) return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold"
        style={{ background: '#FFF3E0', color: '#E65100' }}>
        🟠 {c.giorni}gg
      </span>
    )
    if (c.giorni <= 90) return (
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex items-center gap-3 mt-0.5">
          ...
          <select value={sortMode} onChange={e => setSortMode(e.target.value as SortMode)}
            ...
            <option value="alert">Ordina: per urgenza</option>
            ...
            <tbody>
              {filteredClienti.map((c, i) => {
                ...
                const contrattoLabel = c.tipo_contratto === 'ppl'
                  ? <span className="text-xs px-1.5 py-0.5 rounded font-medium" style={{ background: '#F5F3FF', color: '#6D28D9' }}>PPL{c.lead_obiettivo ? ` ${c.lead_raccolte ?? 0}/${c.lead_obiettivo}` : ''}</span>
                  : <span className="text-xs text-gray-400">Progetto</span>

                return (
                  <tr key={c.id}
                    style={{
                      background: isCritica ? '#FFF8F8' : isEven ? '#FFF' : '#FAFAFA',
                      borderBottom: '1px solid #F0F0F0',
                      borderLeft: isCritica ? '3px solid #E53935' : '3px solid transparent',
                    }}>
                    <td className="px-4 py-3">
                      <button onClick={() => onClienteClick(c.id)}
                        className="font-medium text-sm text-gray-900 hover:text-teal-600 hover:underline transition-colors text-left">
                        {c.nome}
                      </button>
                    </td>
                    <td className="px-4 py-3 text-sm">
                      {referente ? (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-gray-700">{referente.nome.split(' ')[0]}</span>
                        </div>
                      ) : '—'}
                    </td>
                    ...
                    <td className="px-4 py-3 text-sm">
                      {c.prossimaScadenza ? (
                        <span className={isCritica ? 'font-semibold text-red-700' : 'text-gray-700'}>
